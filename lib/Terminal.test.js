    ...
    describe('clear', function () {
        it('should clear a buffer equal to rows', function () {
            ...
            for (var i = 1; i < term.rows; i++) {
                chai_1.assert.deepEqual(term.buffer.lines.get(i), BufferLine_1.BufferLine.blankLine(term.cols, Buffer_1.DEFAULT_ATTR));
            }
        });
        ...
        describe('scroll() function', function () {
            describe('when scrollback > 0', function () {
                it('should create a new line and scroll', function () {
                    term.buffer.lines.get(0).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'a';
                    term.buffer.lines.get(INIT_ROWS - 1).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'b';
                ...
                it('should properly scroll inside a scroll region (scrollTop set)', function () {
                    term.buffer.lines.get(0).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'a';
                    term.buffer.lines.get(1).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'b';
                    term.buffer.lines.get(2).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'c';
                    term.buffer.y = INIT_ROWS - 1;
                ...
                it('should properly scroll inside a scroll region (scrollBottom set)', function () {
                    ...
                    term.buffer.lines.get(1).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'b';
                    term.buffer.lines.get(2).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'c';
                    term.buffer.lines.get(3).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'd';
                    term.buffer.lines.get(4).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'e';
                    term.buffer.y = 3;
            ...
            describe('when scrollback === 0', function () {
                ...
                it('should create a new line and shift everything up', function () {
                    term.buffer.lines.get(0).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'a';
                    term.buffer.lines.get(1).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'b';
                    term.buffer.lines.get(INIT_ROWS - 1).get(0)[Buffer_1.CHAR_DATA_CHAR_INDEX] = 'c';
                    term.buffer.y = INIT_ROWS - 1;
                    chai_1.assert.equal(term.buffer.lines.length, INIT_ROWS);
                    term.scroll();
                    chai_1.assert.equal(term.buffer.lines.length, INIT_ROWS);
